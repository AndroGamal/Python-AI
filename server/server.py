...
class UI:
    ...
    def PickUp(self,d={}):
      ...
      Item_id=d['Item_id']
      Quantity=d['Quantity']
      UI.users[id]['Items'][Item_id]=Quantity

    def PutBack(self,d={}):
      id=d['User_id']
      Item_id=d['Item_id']
      Quantityback=['Quantity']
      UI.users[id]['Items'][Item_id]=UI.users[id]['Items'][Item_id]-Quantityback

    def CheckOut(self,d={}):
      id=d['User_id']
      time=d['CheckOut_time']
      UI.users[id]['TimeCheckout']=time
...
def get():
    ...
    for i in range(0,len(l_user)):
        ...
        out+='''<div class='table'><div class=\\"checkDetails user\\"><h2>Name: '''+str(UI.users[l_user[i]]["Name"])+'''</h2> <h2>'''+str(UI.users[l_user[i]]["TimeCheckin"])+'''</h2></div> <ul><li><span class='list-title'>Item Name</span><span class='list-title text-center'>Quantatiy</span><span class='list-title text-center'>Price</span><span class='list-title text-center'>Total</span></li>'''
        for r in range(0,len(l_item)):
            total=l_item_value[r]*UI.list[l_item[r]][1]
            total_item=+total
            out+='''<li><span>'''+UI.list[l_item[r]][0]+'</span><span class=\\"text-center\\">'+str(l_item_value[r])+'''</span><span class=\\"text-center\\">'''+str(UI.list[l_item[r]][1])+''' €</span><span class=\\"text-center\\">'''+str(total)+'''€</span></li></ul>'''
        if('TimeCheckout' in UI.users[l_user[i]]):
            out+='''<div class=\\"checkDetails total\\"><h2>Check Out Time: '''+str(UI.users[l_user[i]]['TimeCheckout'])+'''</h2><h2  class='text-center price'><span class='sm-appear'>Total Price:</span>'''+ str(total_item)+''' €</h2>'''
        out+='''</div></div><p>&nbsp;<p>'''
    return out
